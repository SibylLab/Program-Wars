...
export default class DryReq extends Requirement {
  ...
  awardBonus (round, completedOnTime, playerDetails) {
    ...
    if (round === 1) {
      ...
      if (completedOnTime) {
        player.deck.cards.unshift(new Card("SCAN", 0))  // Until SEARCH is implemented
      }

    } else if (round === 3) {
      if (completedOnTime) {
        player.deck.cards.unshift(new Card("SCAN", 0))
      }
    }
  }
  ...
  numCompleteMethodStacks (stacks) {
    ...
    const methodStacks = playerDetails.stacks.filter(s => s.getBase().type === 'METHOD')
