...
describe('test store.js mutations', () => {
  ...
  it('test the store addPlayers function', () => {
    ...
    expect(store.getters.getPlayers[0].name).toEqual('joe')
    store.state.activePlayer = 0
    expect(store.getters.currentPlayerName).toEqual('joe')
  })

  it('test the initTutorialDeck function', () => {
    store.commit('initTutorialDeck')
    expect(store.state.tutorialDeck.cards.length).toEqual(66)
  })

  it('test the store changeBonusScore function', () => {
    store.commit('changeBonusScore', {id: 0, score: 1})
    expect(store.getters.getPlayers[0].bonus).toEqual(1)
  })
  ...
  it('test the store giveFirewall function', () => {
    ...
    expect(store.getters.getPlayers[0].hasFirewall).toEqual(true)
  })
  ...
  it('test the store giveGenerator function', () => {
    ...
    expect(store.getters.getPlayers[0].hasGenerator).toEqual(true)
  })
  ...
  it('test the store giveAntiVirus function', () => {
    ...
    store.commit('giveFirewall', 0)
    store.commit('giveGenerator', 0)
    expect(store.getters.getPlayers[0].hasAntiVirus).toEqual(true)
  ...
  it('test the store giveOverclock and giveVirus functions', () => {
    ...
    expect(store.getters.getPlayers[0].hasVirus).toEqual(false)
    expect(store.getters.getPlayers[0].hasOverclock).toEqual(false)
    expect(store.getters.getPlayers[1].hasVirus).toEqual(false)
    expect(store.getters.getPlayers[1].hasOverclock).toEqual(false)
  })

  it('test the store givePowerOutage and giveBatteryBackup functions', () => {
    store.commit('givePowerOutage', 0)
    expect(store.getters.getPlayers[0].hasPowerOutage).toEqual(true)
    store.commit('giveBatteryBackup', 0)
    expect(store.getters.getPlayers[0].hasPowerOutage).toEqual(false)
  })

  it('test the store flipTutorialStep function', () => {
    store.commit('flipTutorialStep')
    expect(store.state.tutorialStep).toEqual(false)
    store.commit('giveBatteryBackup', 0)
    expect(store.getters.getPlayers[0].hasPowerOutage).toEqual(false)
  })
  ...
  it('test the store setFirstRound function', () => {
    ...
    expect(store.state.firstRound).toEqual(false)
  })
  ...
  it('test the setAiTurn function', () => {
    ...
    expect(store.state.aiTurn).toEqual(true)
    store.commit('setAiTurn', false)
    expect(store.state.aiTurn).toEqual(false)
  })

  it('test the increaseFactIndex function', () => {
    expect(store.state.factIndex).toEqual(0)
    store.commit('increaseFactIndex')
    expect(store.state.factIndex).toEqual(1)
  })

  it('test the setPlayfieldColour function', () => {
    store.commit('setPlayfieldColour', false)
    // expect(store.state.trueSideColour).toEqual('background-color: #80aef7; box-shadow: 0px 3px 15px rgba(0,0,0,0.6)')
    // expect(store.state.falseSideColour).toEqual('background-color: #80aef7; box-shadow: 0px 3px 15px rgba(0,0,0,0.6)')
    store.state.activeSide = true
    store.commit('setPlayfieldColour', true)
    expect(store.state.trueSideColour).toEqual('background-color: rgba(14, 183, 99, 0.9); box-shadow: 0 0 15px 10px forestgreen')
    expect(store.state.falseSideColour).toEqual('background-color: rgba(242, 0, 0, 0.4)')
    store.commit('setActiveSide', false)
    store.commit('setPlayfieldColour', true)
    expect(store.state.falseSideColour).toEqual('background-color: rgba(14, 183, 99, 0.9); box-shadow: 0 0 15px 10px forestgreen')
    expect(store.state.trueSideColour).toEqual('background-color: rgba(242, 0, 0, 0.4)')
  })

  it('test the setCoinFlipAnim function', () => {
    expect(store.state.coinFlip).toEqual(0)
    store.commit('setCoinFlipAnim', 1)
    expect(store.state.coinFlip).toEqual(1)
  })

  it('test the playerModalTrigger function', () => {
    expect(store.state.playerTurn).toEqual(false)
    store.commit('playerModalTrigger')
    expect(store.state.playerTurn).toEqual(true)
  })
  ...
  it('test the checkWin function', () => {
    expect(store.state.winner).toEqual(false)
    store.state.players[1].instructions = 100
    ...
    expect(store.state.winner).toEqual(true)
    ...
    store.state.players[1].instructions = 0
    store.commit('checkWin')
    expect(store.state.winner).toEqual(false)
  })
  ...
  it('test the setStackSelectedBoolean function', () => {
    ...
    expect(store.state.selectedStackBoolean).toEqual(false)
  })

  it('test the removeAllSelectedStacks function', () => {
    store.commit('removeAllSelectedStacks')
    expect(Array.isArray(store.state.selectedStacks)).toEqual(true)
  })
  ...
  it('test the discardSelectedCard function', () => {
    ...
    store.commit('discardSelectedCard')
    expect(store.state.deck.discard_cards[0].id).toEqual(55)
    expect(store.state.tutorialDeck.discard_cards[0].id).toEqual(55)
  })
  ...
  it('test the removeActiveCardFromHand function', () => {
    ...
    expect(store.state.activeCard).toEqual(undefined)
  })
  ...
  it('test the setActiveCard function', () => {
    ...
    expect(store.state.activeCard).toEqual(55)
  })
  ...
  it('test the addCardToHand function', () => {
    ...
    expect(store.state.hands[0].cards.length).toEqual(6)
    ...
    store.state.isTutorial = true
    store.commit('addCardToHand')
    expect(store.state.hands[1].cards.length).toEqual(0)
    store.state.tutorialDeck.cards = []
    store.state.activePlayer = 1
    store.commit('addCardToHand')
    expect(store.state.hands[1].cards.length).toEqual(6)
    store.commit('initTutorialDeck')
  })
  ...
  it('test the addHandToPlayer function', () => {
    ...
    expect(store.state.players[0].hand.length).toEqual(36)
    store.state.isTutorial = true
    store.commit('addHandToPlayer', 1)
    expect(store.state.players[1].hand.length).toEqual(36)
  })
  ...
  it('test the stackDiscard function', () => {
    ...
    expect(store.state.deck.discard_cards.length).toEqual(1)
    store.state.isTutorial = true
    store.commit('stackDiscard', {stackId: store.state.stacks[0].stackId})
    expect(store.state.tutorialDeck.discard_cards.length).toEqual(1)
  })
  ...
  it('test the getIsLast function', () => {
    ...
    store.state.activePlayer = 0
    store.commit('getIsLast')
    expect(store.state.isLast).toEqual(false)
  })

  it('test the setActiveStack function', () => {
    store.commit('setActiveStack', store.state.stacks[0])
    expect(store.state.activeStack.stackId).toEqual(store.state.stacks[0].stackId)
  })
})
